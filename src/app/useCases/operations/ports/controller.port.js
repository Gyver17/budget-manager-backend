...
class controllerPort {
	...
	async operations(req, res) {
		...
		const { status, json } = await this.controller.operations(userId);
		return this.adapter.response(res, status, json);
	}

	async operationsHomeValue(req, res) {
		const { userId } = this.adapter.params(req);
		const { status, json } = await this.controller.operationsHomeValue(
			userId
		);
		return this.adapter.response(res, status, json);
	}
	...
	async updateOperation(req, res) {
		const { id } = this.adapter.params(req);
		const data = this.adapter.body(req);
		const { status, json } = await this.controller.updateOperation(
			id,
			data
		);
		return this.adapter.response(res, status, json);
